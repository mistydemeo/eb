...
old_archive_cmds='$AR $ARFLAGS $oldlib$oldobjs$old_deplibs'
...
EOF
  ...
  amigaos*)
    archive_cmds='$rm $output_objdir/a2ixlibrary.data~$echo "#define NAME $libname" > $output_objdir/a2ixlibrary.data~$echo "#define LIBRARY_ID 1" >> $output_objdir/a2ixlibrary.data~$echo "#define VERSION $major" >> $output_objdir/a2ixlibrary.data~$echo "#define REVISION $revision" >> $output_objdir/a2ixlibrary.data~$AR $ARFLAGS $lib $libobjs~$RANLIB $lib~(cd $output_objdir && a2ixlibrary -32)'
...
EOF
  ...
  *)
  ...
  aix3*)
    allow_undefined_flag=unsupported
    always_export_symbols=yes
    archive_expsym_cmds='$LD -o $output_objdir/$soname $libobjs $deplibs $linker_flags -bE:$export_symbols -T512 -H512 -bM:SRE~$AR $ARFLAGS $lib $output_objdir/$soname'
...
if test -f "$ltmain"; then
  ...
  for var in echo old_CC old_CFLAGS \
    AR ARFLAGS CC LD LN_S NM SHELL \
